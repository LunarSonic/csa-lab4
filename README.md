...
- Поддерживается одностороннее неявное приведение типов из `int` в `long`
  - Если в выражении, где ожидается тип `long`, используется переменная или литерал типа `int`, транслятор автоматически расширяет значение до 64 бит путём добавления нулевого старшего слова при помещении на стек
  - Обратное приведение (`long -> int`) не поддерживается
...
| `latch tmp1`, `latch tmp2` | Запись значения с выхода мультиплексоров во временные регистры операндов                         |
| `latch AR`                 | Фиксация вычисленного адреса для последующего обращения к памяти данных                          |
| `sel tmp1`, `sel tmp2`     | Выбор источника данных (регистр, память через кеш или константа из инструкции)                   |
| `sel addr`                 | Выбор источника адреса: прямое значение из регистра или результат сложения регистра со смещением |
| `sel op`                   | Выбор конкретной операции АЛУ (код операции)                                                     |
| `sel write_data`           | Выбор данных для обратной записи: результат АЛУ, данные из кеша или данные из регистра           |
| `calc op`                  | Сигнал выполнения операции в АЛУ и обновления флагов состояния                                   |
| `store`, `load`            | Сигналы управления кешем для инициирования операций чтения из памяти данных или записи в неё     |
| `read`, `write`            | Сигналы управления контроллером ввода-вывода: чтение из порта или запись в порт                  |
| `mem_store`, `mem_load`    | Сигналы от кеша, которые инициируют чтение из памяти данных или запись в неё                     | 
...
| `latch IR`        | Защёлкивание новой команды из памяти инструкций                                                               |
| `latch PC`        | Обновление счётчика команд (инкремент или переход по адресу)                                                  |
| `latch mPC`       | Сигнал перехода к следующей микрокоманде                                                                      |
| `sel PC`          | Выбор следующего значения PC: адрес перехода из DataPath или инкремент на размер инструкции                   |
| `sel mPC`         | Выбор между следующей микрокомандой или прыжком на адрес начала микропрограммы команды из Instruction Decoder |
| `sel condition`   | Выбор типа проверяемого условия для блока Condition Decider                                                   |
| `check condition` | Сигнал активации проверки условий перехода на основе текущих флагов АЛУ                                       |
| `load`            | Сигнал чтения машинного кода из памяти инструкций                                                             |
...
2. **FETCH_SRC / FETCH_DST**: 1–2 такта
    ...
    - Immediate (2 такта):
        ...
        - Такт 2: Переход на стадию обработки второго операнда (`MPC_DISPATCH_DST`)
    - Indirect (2 такта):
