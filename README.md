...
- Реализация инструкций для записи символов и `\0` в память данных. Реализация циклов print и read с остановкой по нуль-терминатору
...
- Вся микропрограмма хранится в массиве `MICROPROGRAM`, который является последовательностью таких наборов
...
Исполнение инструкции в `Control Unit` разбито на логические стадии, управляемые через микропрограммный счётчик `mPC`. Каждый этап объединяет группу микроинструкций по фиксированным адресам в управляющей памяти
1. **FETCH**: Чтение машинного кода из памяти команд, защёлкивание в `IR`. Инкремент `PC` на размер инструкции
2. **FETCH_SRC**: Извлечение первого операнда в `TMP1`. Микропрограмма выбирается на основе `src_mode`. При необходимости выполняется доступ к памяти данных
3. **FETCH_DST**: Извлечение второго операнда в `TMP2` на основе `dst_mode`
4. **EXECUTE**: Выполнение основной операции АЛУ (арифметика, логика, переходы или ввод-вывод). Обновление флагов состояния процессора
5. **WRITE_BACK**: Сохранение результата работы АЛУ в целевой регистр или ячейку памяти данных. Сброс `mPC` в 0 для перехода на следующую инструкцию
